function infocapacity_hopf_errorhete_hc_tp2(s,condition)
...
if condition==1
      G=optG_hc_tp2;  %Optimal G weight for healthy controls, manual input
end
...
load(sprintf('results_f_diff_fce_cond%d_ON_hc_tp2.mat',condition)); % load frequency with maximal power (of narrowly filtered data) by area computed in script Compute_Hopf_Freq_hc.m
load(sprintf('empirical_spacorr_rest_cond_%d_ON_hc_tp2.mat',condition)); %load empirical FC computed in script Empirical_corrfcn_hc.m
...
save(sprintf('Wtrials_%03d_%d_%s_ON_hc_tp2.mat',s,condition,measure),'infocapacity','susceptibility');
