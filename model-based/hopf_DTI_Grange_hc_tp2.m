...
function hopf_DTI_Grange_hc(s,condition)

%load data
load(sprintf('results_f_diff_fce_cond%d_ON_hc.mat',condition)); % load frequency with maximal power (of narrowly filtered data) by area computed in script Compute_Hopf_Freq_hc.m
load(sprintf('empirical_spacorr_rest_cond_%d_ON_hc.mat',condition)); %load empirical FC computed in script Empirical_corrfcn_hc.m
...
load('tseries_openneuro_controls_tp2_sch1000.mat'); %load empirical time-series
...
%% Parameters HOPF
Tmax=144;
omega = repmat(2*pi*f_diff',1,2); omega(:,1) = -omega(:,1); %angular velocity
...
save(sprintf('WG_%03d_%d_ON_hc.mat',s,condition),'InfoCascade','InfoFlow','Turbulence','err_hete');
