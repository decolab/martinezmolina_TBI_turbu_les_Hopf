...
cd('F:\turbulence\timeseries\pre_post_schaefer1000\inputs');
...
for i=1:NPARCELLS
    for j=1:NPARCELLS
        rr(i,j)=norm(SchaeferCOG(i,:)-SchaeferCOG(j,:)); %matriz de distancia entre centroides de nodos
    end
end
range=max(max(rr));
delta=range/NR;
...
entropy_su=zeros(NLAMBDA,NPARCELLS,Tmax);
...
for sub=1:NSUB
    ...
    for ii=1:7
        ...
        for ilam=1:NLAMBDA
            C1lam=squeeze(C1(ilam,:,:));
            sumphases=nansum(repmat(C1lam(i,:)',1,Tmax).*complex(cos(Phases),sin(Phases)))/nansum(C1lam(i,:));
            entropy2{ilam,ii,i,:}=abs(sumphases);
        end
        end
    end
    TransferLambda_sub(1,:)=NaN;
    ...
    for ii=1:7
        ...
        %TODO: turbulence_nodal_sub(ilam,sub,inode)
        %entropy(ilam,:,:) = entropy1(ilam,inode,:); %Kuramoto module
