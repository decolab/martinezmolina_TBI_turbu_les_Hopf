clear ()
close ()
addpath('/Volumes/LASA/TBI_OpenNeuro/Shared_code/TBI_ON_Lesions/')
data_path='/Volumes/LASA/TBI_OpenNeuro/TBI_openneuro/data/lesions/overlap_Schaefer/Schaefer100_wotbi09/';
cd (data_path)
table_overlap_volume=import_overlap_volume_file('overlap_volume_combined.csv');
overlap_vol_all_patients=table_overlap_volume.VarName1; % N=10 patients with lesion masks
table_Schaefer_volume=import_schaefer_volume_file('Schaefer_volume_combined.csv');
Schaefer_volume=table_Schaefer_volume.VarName1;

%Calculate percent overlap with Schaefer parcel
Schaefer_parcel=reshape(repmat(1:100,10,1), [1000 1]);
Schaefer_volume=reshape(repmat(Schaefer_volume',10,1), [1000 1]);
...
lesion_mask_1andhalfSD=ones(100,100);
...
lesion_mask_1andhalfSD_weighted=ones(100,100);
...
lesion_mask_2SD_weighted=ones(100,100);
for n=1:numel(nodesattack_excludingrep)
    range_weights=(1:1:max(frequency));
    for w=1:numel(range_weights)
        if frequency(n)==range_weights(w)
...
lesion_mask_3SD_weighted=ones(100,100);
for n=1:numel(nodesattack_excludingrep)
    range_weights=(1:1:max(frequency));
    for w=1:numel(range_weights)
        if frequency(n)==range_weights(w)
...
lesion_mask_4SD_weighted=ones(100,100);
for n=1:numel(nodesattack_excludingrep)
    range_weights=(1:1:max(frequency));
    for w=1:numel(range_weights)
        if frequency(n)==range_weights(w)
