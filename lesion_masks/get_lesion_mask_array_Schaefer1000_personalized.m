...
% Create binary lesion mask to attack connectivity matrix

lesion_mask_1andhalfSD_sub=ones(10,1000,1000);
lesion_mask_1andhalfSD_sub(sbj,:, idx_1andhalfstd)=0; lesion_mask_1andhalfSD_sub(sbj,idx_1andhalfstd, :)=0;
figure
imagesc(squeeze(lesion_mask_1andhalfSD_sub(sbj,:,:))); colorbar
...
figure
imagesc(squeeze(lesion_mask_2SD_sub(sbj,:,:))); colorbar
...
figure
imagesc(squeeze(lesion_mask_3SD_sub(sbj,:,:))); colorbar
...
figure
imagesc(squeeze(lesion_mask_4SD_sub(sbj,:,:))); colorbar
...
%% Add subjects with no lesion mask (sub-01 & sub-09)
