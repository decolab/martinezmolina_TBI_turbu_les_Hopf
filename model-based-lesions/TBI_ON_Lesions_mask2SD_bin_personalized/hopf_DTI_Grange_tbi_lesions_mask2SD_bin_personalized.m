...
function hopf_DTI_Grange_tbi_lesions_mask2SD_bin_personalized(s,condition)
...
load('lesion_mask_2SD_sub_N12.mat')
...
for sbj=1:NSUB
    ...
    empcorrfcn=squeeze(corrfcnsub(sbj,:,:)); % CHANGED!!! subject-specific empirical fcn
    ...
    C=C.*squeeze(lesion_mask_2SD_sub_N12(sbj,:,:)); % CHANGED!!! subject-specific lesion mask from tbi patients
    ...
    for sub=1:NSUBSIM
        ...
        %%
        Turbulence(sub)=Rsub;
        save(sprintf('WG_%03d_%d_sub_%d_ON_tbi_mask2SD_bin.mat',s,condition,sbj),'InfoCascade','InfoFlow','Turbulence','err_hete');
    end
end