...
% Load inputs
clear all
cd('/Volumes/LASA/TBI_OpenNeuro/TBI_openneuro/timeseries/inputs_sch400/');                                % Structural connectivity provided by Morten Kringelbach
load('tseries_openneuro_hc_tp2_sch400.mat')               % Denoised time-series
load ('RSN7_labels_sch400.mat');                                         % Labels for the 7 RSN (Yeo, 2011)
load('SchaeferCOG_sch400.mat');                                         % Center of gravity coordinates in the Schaefer parcellation
labels= RSN_labels;
...
NPARCELLS=400;                             % Number of nodes in Schaefer's parcellation
NR=400;                               
NRini=20;
NRfin=80;
...
for i=1:NPARCELLS
    for j=1:NPARCELLS
        rr(i,j)=norm(SchaeferCOG_sch400(i,:)-SchaeferCOG_sch400(j,:)); % Returns the Euclidean norm of vector v or Euclidean length.
    end
end
range=max(max(rr));
delta=range/NR;
...
for sub=1:NSUB
    ...
    %     % Calculate amplitude turbulence for all nodes and timepoints
        for ilam=1:NLAMBDA
    ...
    for ii=1:7
        ...
        gKoPRSN(sub,ii)=nanmean(abs(sum(complex(cos(Phases(indx_rsn,:)),sin(Phases(indx_rsn,:))),1))/NPARCELLS_RSN); %global Kuramoto parameter (synchronization) for each network
        MetaRSN(sub,ii) = nanstd(abs(sum(complex(cos(Phases(indx_rsn,:)),sin(Phases(indx_rsn,:))),1))/NPARCELLS_RSN); %global metastability for each network
    end
...
cd ('/Volumes/LASA/TBI_OpenNeuro/TBI_openneuro/timeseries/outputs/Turbulence_12_07_2023/')
save (sprintf('turbu_all_measurements_openneuro_hc_tp2_sch400_con%d.mat',cond), 'LAMBDA','TurbulenceRSN_sub','Turbulence_global_sub', 'InformationCascade_sub', 'TransferLambda_sub','Transfer_sub','gKoPRSN','MetaRSN','gKoP','Meta');